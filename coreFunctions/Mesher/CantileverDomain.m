...
%-------------------------------------------------------------------------%
function [x] = CantileverDomain(Demand,dim,Arg)
switch dim
    case 2
        BdBox = [0 2 0 1];
    case 3
...
switch dim
    case 2
        ...
        RigthMidNode = find(abs(Node(:,1)-BdBox(2))<eps & ...
            abs(Node(:,2)-(BdBox(3)+BdBox(4)/2))<eps);
        FixedNodes = LeftEdgeNodes;
        Supp = ones(length(FixedNodes),3);
        Supp(:,1)=FixedNodes; 
        Load = [RigthMidNode,0,-1];
    case 3
        ...
        Load = [LeftUpperNode,ones(size(LeftUpperNode))*[0,-1,0]];
end
x = {Supp,Load};
