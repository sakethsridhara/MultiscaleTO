...
%% Optimize
function OptimizeDirect(dType, msType, deg, volFrac)
close all;
...
[coeff,derCoeff] = getCdirectCoeff(msType,deg);
%  Start of analysis
[Cs,gradCs] = getCdirectInterp(density,coeff,derCoeff);
...
for iter = 1:maxIter                    %Start iterations
    ...
    if(change(iter)<0.001)
        disp('Change in density converged');        break;
    end
...
%--------------------------------------------%
function K_elem = getKElem(Ke_template,Cs)
K_elem = zeros(8,8,size(Cs,3));
for comp = 1:size(Cs,1)
    K_elem = K_elem+Ke_template(:,:,comp).*Cs(comp,1,:);
end
%--------------------------------------------%
